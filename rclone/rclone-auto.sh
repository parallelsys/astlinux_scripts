...
#
# 1) Modification added for astlinux persistant storage. Change to your owned prefered location.
#
# 2) Backup Source
...
# 
# 5) You can change the bandwidth limits by editing BWLIMT, which includes a timetable facility.  
...
DSTCONF=/mnt/kd/.rclone/rclone.conf
...
LOGFILE=$SRC/.rclone/rclone-copy.log
...
exit_on_lock()      { echo $(date -u)' | rclone-auto.sh already running.' >> $CRONLOG; exit 1; }
...
#Log startup
echo $(date -u)' | starting rclone-auto.sh . . .' >> $CRONLOG

#Now do the copy!
rclone --config $DSTCONF --fast-list sync $SRC $DEST --transfers $TRANSFERS --bwlimit "$BWLIMIT" --min-age $MINAGE --exclude-from $EXCLUDEFILE --exclude-if-present $EXIFPRESENT --delete-excluded $LOGS


#log success
echo $(date -u)' | completed rclone-auto.sh.' >> $CRONLOG
